...
import market.exceptions.UserNotFoundException;
...
public class Market {
    ...
    public int makeOrder(User user, Basket basket, Double finalPrice) throws UserNotFoundException {
        if (!usersList.contains(user)) throw new UserNotFoundException("user not found, " + user);
    ...
    public void backup(String source, String target) {
        ...
        backup.copyFilesInPath(source, target);
    }
